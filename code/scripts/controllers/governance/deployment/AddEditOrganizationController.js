  ...
  attachViewEventListeners() {
    ...
    this.onTagClick('organization.submit', async (model, target, event) => {
      ...
      try {
        ...
        if (this.model.isEditing) {
          ...
          await this.sharedStorageService.insertRecordAsync(constants.TABLES.GOVERNANCE_ORGANIZATIONS, utils.getPKFromCredential(submitModel.uid), submitModel);
        }
    ...
    this.onTagClick('organization.remove', async (model, target, event) => {
      ...
      try {
        ...
        await this.ui.showToast(`Organization ${submitModel.name} has been removed!`);
        this.goBackToOrganizationsDashboard();
      } catch (e) {
        await this.ui.showToast(`Encountered error: ` + e.message);
      }
